...
classifier = keras.models.load_model('Model_5(2_conv(64,64)+0_Dense+Last_Dense)')
...
for i in range(len(X_test)):
    if predictions[i][0] > 0.4 and y_test[i] == 0 :
...
#        plt.imshow(Training_data[i][0], cmap="gray")
    ...
    elif predictions[i][0] <= 0.4 and y_test[i] == 1 :
        plt.imshow(Training_data[len(Training_data)-50+i][0], cmap="gray")
#        plt.imshow(Training_data[i][0], cmap="gray")
        plt.show()
        print(predictions[i][0])
